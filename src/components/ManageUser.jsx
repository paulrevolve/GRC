...
export default function ManageUsers() {
  ...
  const fetchDepartments = async () => {
    ...
    try {
      ...
      if (mappedDepartments.length > 0 && !selectedDepartmentId) {
        setSelectedDepartmentId(String(mappedDepartments[0].id));
      }
    } catch (err) {
  ...
  const handleSaveUser = async (e) => {
    ...
    if (!editingUserId) {
      ...
      if (!isStrongPassword(trimmedPassword)) {
        toast.warning(
          `Password must be at least ${MIN_PASSWORD_LENGTH} characters and include a letter, a number, and a special character.`,
        );
        return;
      }
    ...
    setIsSaving(true);
    const numericDepartmentId = selectedDepartmentId
      ? Number(selectedDepartmentId)
      : null;
    const numericRoleId = userRoleInput ? Number(userRoleInput) : null;

    try {
      if (editingUserId) {
        // Matches updated API payload with roleIds array
        const updatePayload = {
          ...
          organizationId: 1,
          isActive: Boolean(userIsActive),
          roleIds: numericRoleId ? [numericRoleId] : [],
        };
  ...
  const handleEdit = (user) => {
    setEditingUserId(user.userId);
    setUserNameInput(user.userName || "");
    setUserFullNameInput(user.displayName || "");
    setUserEmailInput(user.email || "");
    setSelectedDepartmentId(user.departmentId ? String(user.departmentId) : "");
    setUserIsActive(user.isActive ?? true);

    // Extract role ID if roles array exists on the user object
    if (user.roleIds && user.roleIds.length > 0) {
      setUserRoleInput(String(user.roleIds[0]));
    } else if (user.roles && user.roles.length > 0) {
      setUserRoleInput(String(user.roles[0].id || user.roles[0]));
    } else if (roles.length > 0) {
      setUserRoleInput(String(roles[0].id));
    }
  ...
  const handleResetPasswordSubmit = async (e) => {
    ...
    if (!isStrongPassword(trimmedNewPassword)) {
      setPasswordError(
        `Password must be at least ${MIN_PASSWORD_LENGTH} characters and include a letter, number, and special character.`,
      );
      return;
    }
                        ...
                        <td className="px-3 py-2">
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="text-slate-400 hover:text-amber-600 cursor-pointer hover:bg-amber-50 rounded p-1"
                              title="Reset Password"
                            >
                              <Key size={13} />
                            </button>
                            <button
                              ...
                              className="text-slate-400 hover:text-blue-600 cursor-pointer hover:bg-blue-50 rounded p-1"
                              title="Edit User"
                            >
                              <Edit2 size={13} />
                            </button>
                            <button
                              ...
                              className="text-slate-400 cursor-pointer hover:text-red-600 hover:bg-red-50 rounded p-1"
                              title="Delete User"
                            >
                              <Trash2 size={13} />
                            </button>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setUserNameInput(e.target.value)}
                    placeholder="e.g. jdoe"
                    className="w-full text-xs border border-slate-200 rounded px-2 py-1.5 outline-none focus:border-[#0F3A46] disabled:bg-slate-100"
                  />
                </div>
